...
int main(int argc, const char * argv[]){
    ...
    double frequency = 1.0; //frequency constant for sine wave
    ...
    for (i = 0; i < H_ROW; i++)
        for (j = 0; j < H_COL; j++){
            fprintf(out_file, "%d ", (int)((float) h[i][j]*(1.0 + (float) i/23.0))); //write ppm pixel data (R)
            fprintf(out_file, "%d ", h[i][j]*(H_COL - j)/20); //write ppm pixel data (G)
